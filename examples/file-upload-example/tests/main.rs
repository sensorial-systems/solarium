...
async fn solana() -> Result<()> {
    ...
    println!(
        "Preparing upload: {} bytes, expected CRC32=0x{:08x}",
        size, expected_crc32
    );
    ...
    println!(
        "Initializing header (payload_len=0, file_size={})...",
        file_size
    );
    ...
    builder
        .message
        .instructions
        .push(ComputeBudgetInstruction::set_compute_unit_limit(1_400_000));
    ...
    let account_data = program
        .connection()
        .get_account_data(&file_pda.address())
        .await?;
